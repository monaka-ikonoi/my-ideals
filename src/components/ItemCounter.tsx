...
export function ItemCounter({ value, setValue, mode = 'normal' }: ItemCounterProps) {
    ...
    >
      <button
        ...
        className={`flex h-full items-center justify-center text-gray-500 transition
          ...
          ${mode === 'edit' ? 'w-[50%]' : 'w-[28%] max-w-8'}`}
        onClick={() => setValue(value - 1)}
      >
        <MinusIcon className={mode === 'edit' ? 'h-5 w-5' : 'h-4 w-4'} />
      </button>

      {mode !== 'edit' && (
        <div
          className={`h-full min-w-0 flex-1 items-center justify-center text-base font-semibold
      ...
      <button
        type="button"
        className={`flex h-full items-center justify-center text-gray-500 transition
          hover:bg-gray-200 active:bg-gray-300 ${mode === 'edit' ? 'w-[50%]' : 'w-[28%] max-w-8'}`}
        onClick={() => setValue(value + 1)}
      >
        <PlusIcon className={mode === 'edit' ? 'h-5 w-5' : 'h-4 w-4'} />
      </button>
    </div>
  );
}
